export type AdditionalPaymentStrategy = 'shorten-duration' | 'reduce-payment'
...
const calculateFrenchMonthlyPayment = (principal: number, monthlyRate: number, months: number) => {
...
export function calculateMortgagePlan(input: MortgageInput): MortgagePlan {
  ...
  for (let month = 1; month <= totalMonths && remainingPrincipal > 0; month += 1) {
    ...
    if (additionalPaymentStrategy === 'reduce-payment' && extraPrincipalPaid > 0 && remainingPrincipal > 0) {
      ...
      monthlyRateWithoutBankCost = calculateFrenchMonthlyPayment(
        remainingPrincipal,
        monthlyRate,
        remainingMonthsAfterCurrent,
      )
    }
